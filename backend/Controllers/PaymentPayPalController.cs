...
namespace backend.Controllers
{
    ...
    public class PaymentPayPalController : ControllerBase
    {
        ...
        public PaymentPayPalController(
            ...
            IMapper mapper,
            Search_TourDetail_Dao searchDao,
                ...
                purchase_units = new List<PurchaseUnitInput>
                {
                    new PurchaseUnitInput
                    {
                        ...
                        amount = new AmountInput
                        {
                            currency_code = "USD",
                            value = (
                                int.Parse(orderDetail.Price) * int.Parse(orderDetail.quantity)
                            ).ToString(),
        ...
        public async Task<IActionResult> CapturePayment1(PaymentPayPalDtos payment)
        {
            ...
            var captureData = new
            {
                note_to_payer = "Capture payment for order " + payment.orderid
            }; // Thay thế bằng thông tin cần thiết
            ...
            using (var transaction = context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        var exist_tour_detail = await searchDao.QueryDao(
                            payment.Tour_Detail_Payment_Dto.Start_Date,
                            payment.Tour_Detail_Payment_Dto.TourId
                        );

                        if (exist_tour_detail == null)
                        {
                            var tour_detail = mapper.Map<Tour_Detail_PaymentPaypal_Dto, TourDetail>(
                                payment.Tour_Detail_Payment_Dto
                            );
                            exist_tour_detail = await TourDetailBusinessLogic.Create(tour_detail);
                        }

                        var response_capture = await _client.GetAsync(
                            $"https://api-m.sandbox.paypal.com/v2/checkout/orders/{payment.orderid}"
                        );
                        var responseAsString_capture =
                            await response_capture.Content.ReadAsStringAsync();
                        var result = await GetCapturePayment(responseAsString_capture);

                        var check_duplicate_order = await OrderBusinessLogic.GetEntityByCondition(
                            exist_tour_detail.Id
                        );
                        ...
                        var oderdetail = new OrderDetail
                        {
                            ...
                            Description =
                                result.purchase_units[0].items[0].description
                                + " |Paypal fee: "
                                + Double.Parse(
                                    result.purchase_units[0].payments.captures[0]
                                        .seller_receivable_breakdown
                        ...
                        var totalOrderPrice = list_orderdetail.Sum(
                            orderDetail => orderDetail.Price
                        );
                        var totalOrderQuantity = list_orderdetail.Sum(
                ...
                catch (Exception ex)
                {
                    ...
                    throw new Exception(
                        "You have successfully paid but encounter problems during processing, please contact management",
                        ex
                    );
                }
            }
